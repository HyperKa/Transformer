...
### Бизнес-задача: ускорение и облегчение статического анализа
...
1. Если в корне проекта нет .tar-контейнера - соберите проект и упакуйте готовый образ в один переносимый архив .tar:

    `docker save -o java-parser-container.tar transformer_gpu-java-parser:latest`
2. Скопируйте на сервер следующие файлы: 
   1) Файл образа java-parser-container.tar
   2) Конфигурационные файлы: docker-compose.yml, .env (с ключами Mistral API)
   3) Папку с сырыми данными datasets/raw/
   4) На сервере импортируйте образ в Docker:
   
    `docker load -i java-parser-container.tar`
3. Перед первым запуском на сервере обязательно создайте пустые файлы для монтирования томов. Если этого не сделать, Docker автоматически создаст папки с такими именами, что приведет к ошибкам доступа в Java:
    
    `touch checkpoint.txt final_training_dataset.txt`
4. Запустите службу разметки в фоновом режиме (демоне):

    `docker compose up -d`
5. Для контроля процесса генерации датасета используйте стандартные команды Docker Compose внутри папки проекта на сервере:

    `docker compose logs -f java-parser` - логи парсера, если вдруг VPN-соединение прервется
